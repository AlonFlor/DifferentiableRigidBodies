...
def run_one_time_step(dt, shapes, combined, shape_shape_frictions, shape_ground_frictions, fixed_contact_shapes, find_derivatives, external_forces, locations_records=None, motion_script_to_write=None, energies_records=None, time=None, motion_script_at_current_step=None):
    ...
    for external_force in external_forces:
        external_force_magn, component_number, direction = external_force
        external_force_contact_location = collision_handling_basic_impulse.external_force_impulse(combined, external_force_magn, component_number, direction, dt, find_derivatives)
        print("\texternal force:",external_force_magn, "at", external_force_contact_location, "pointing towards", direction)
    print("angular velocity before friction", combined.angular_velocity)
    combined.set_component_velocities_and_angular_velocities()

    #if writing_motion_script:
    #    file_handling.write_external_force_info_in_motion_script(external_force_magn, external_force_contact_location, direction_z, motion_script_to_write)

    #handle collisions
    #collision_handling_LCP.handle_collisions_LCP(combined, ground_contacts_low_level, dt)
    friction = collision_handling_basic_impulse.handle_collisions_using_impulses(shapes, ground_contacts_low_level, find_derivatives, dt, ground_contact_friction_coefficients)
    print("\n\n")
    return friction
...
def make_shapes(shape_masses, shape_ground_frictions_in):
    # make shapes
    ...
    combined = combined_body(shapes, np.array([-0., 0., 0.]), np.array([0., 0., 0.]))
    return shapes, shape_shape_frictions, shape_ground_frictions, combined

def try_a_force_pair(shape_masses, shape_ground_frictions_in, first_force, second_force):
    # make shapes
    shapes, shape_shape_frictions, shape_ground_frictions, combined = make_shapes(shape_masses, shape_ground_frictions_in)

    friction = run_one_time_step(dt, shapes, combined, shape_shape_frictions, shape_ground_frictions, combined.fixed_contacts, False, [first_force, second_force])

    return combined.velocity, combined.angular_velocity[1], friction
...
def find_balancing_external_force_pair(shape_masses, shape_ground_frictions_in, first_force_magn, component1, dir1, component2, dir2):
    ...
    velocity_result, angular_velocity_result, friction = try_a_force_pair(shape_masses, shape_ground_frictions_in, [first_force_magn, component1, dir1], [second_force_magn, component2, dir2])
    ...
    while np.abs(result[2]) > 1e-9:  # threshold
        ...
        velocity_result, angular_velocity_result, friction = \
            try_a_force_pair(shape_masses, shape_ground_frictions_in, [first_force_magn, component1, dir1], [second_force_magn, component2, dir2])
...
def mass_and_com_one_axis(shape_masses, shape_ground_frictions_in, component1, dir1, component2, dir2, first_force_along_axis, second_to_first_force_along_axis, velocity_axis):
    ...
    # get com coord along the given axis
    ...
    translation_only_motion_frictions_z = []
    ...
    for friction in translation_only_motion_frictions:
        ...
        translation_only_motion_frictions_z.append(friction[2])
    ...
    print("\tz-axis friction")
    draw_data.plot_data(first_force_magns, translation_only_motion_frictions_z)
...
def mass_moments_finder(shape_masses, shape_ground_frictions_in):
    ...
    com_x, force_magn_pairs, angular_velocity_changes_divided_by_dt = \
        mass_and_com_one_axis(shape_masses, shape_ground_frictions_in, component1, dir1, component2, dir2, first_force_rx, second_to_first_force_rx, 2)

    #third force and fourth forces
    third_force_rz = external_force_contact_location_first_force[2]
    dir3 = [1., 0., 0.]
    dir4 = [1., 0., 0.]
    component3 = 27
    shapes, shape_shape_frictions, shape_ground_frictions, combined = make_shapes(shape_masses, shape_ground_frictions_in)  # make shapes
    external_force_contact_location_fourth_force = geometry_utils.to_world_coords(combined.components[component3], combined.components[component3].vertices[0]) * np.array([1., 0., 1.])
    fourth_force_rz = external_force_contact_location_fourth_force[2]
    fourth_to_third_force_rz = third_force_rz - fourth_force_rz

    com_z, force_magn_pairs_unused, angular_velocity_changes_divided_by_dt_unused = \
        mass_and_com_one_axis(shape_masses, shape_ground_frictions_in, component1, dir3, component3, dir4, third_force_rz, fourth_to_third_force_rz, 0)

    actual_com = geometry_utils.to_world_coords(combined, combined.COM)
    print("\n\ncenter of mass result:")
    print("\t", com_x, com_z)
    print("actual value:")
    print("\t", actual_com[0], actual_com[2])


    print("\n\n\n\nprocessing moment of inertia using the found center of mass")
    pushing_torques = []
    found_com = np.array([com_x, external_force_contact_location_first_force[1], com_z])
    com_to_first_force = external_force_contact_location_first_force - found_com
    com_to_second_force = external_force_contact_location_second_force - found_com
    for force_magn_pair in force_magn_pairs:
        first_force_magn, second_force_magn = force_magn_pair
        pushing_torques.append(np.cross(com_to_first_force, np.array(dir1)*first_force_magn)[1] + np.cross(com_to_second_force, np.array(dir2)*second_force_magn)[1])
...
def center_of_friction(combined, shape_ground_frictions):
    center = np.array([0., 0., 0.])
    amount = len(combined.components)
    for i in np.arange(amount):
        center += combined.components[i].location * shape_ground_frictions[i] * combined.components[i].mass
    center /= amount
    return center
...
for i in np.arange(33,75):
    actual_mu_values[i] = 0.2
