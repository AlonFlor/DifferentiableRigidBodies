    ...
    if len(shape_shape_contacts) > 0 or len(ground_contacts) > 0:
        ...
        while len(shape_shape_contacts_with_negative_dv) > 0 or len(shape_ground_contacts_with_negative_dv) > 0:
            ...
            for index, dv in shape_ground_contacts_with_negative_dv:
                ...
                if find_derivatives:
                    impulse, r, I_inv, impulse_mass_derivatives, r_mass_derivatives, I_inv_mass_derivatives = shape_ground_collision_impulse(shape, contact, dv, restitution, True)
                    velocity_mass_derivatives, angular_velocity_mass_derivatives = shape_ground_apply_impulse(shape, impulse, r, I_inv, impulse_mass_derivatives, I_inv_mass_derivatives)
                    ...
                    impulse, r, I_inv = shape_ground_collision_impulse(shape, contact, dv, restitution, False)
    ...
    for i in np.arange(len(ground_contacts)):
        ...
        if find_derivatives:
            ...
            for j in np.arange(len(shapes)):
                normal_impulse_magn_mass_derivatives.append(1. / normal_impulse_magn * np.linalg.norm(
                    ground_contact_impulses[i] * ground_contact_impulses_mass_derivatives[i][j]))
        ...
        friction_direction = tangential_velocity / tangential_velocity_magn
        if find_derivatives:
            relative_motion_friction, r, I_inv, one_shape_friction_impulse_mass_derivatives, one_shape_friction_r_mass_derivatives, one_shape_friction_I_inv_mass_derivatives = shape_ground_collision_impulse(
                shape, (world_point, friction_direction), tangential_velocity_magn, 0., True)
            ...
            for j in np.arange(len(shapes)):
                relative_motion_friction_magn_mass_derivatives.append(
                    1. / relative_motion_friction_magn * relative_motion_friction * friction_impulse_mass_derivatives[j])
            mu_normal_friction_magn = mu * normal_impulse_magn
        ...
        '''else:
            if find_derivatives:
                velocity_mass_derivatives, angular_velocity_mass_derivatives = shape_ground_apply_impulse(shape.parent, friction, r, I_inv, friction_mass_derivatives, friction_I_inv_mass_derivatives)
                for j in np.arange(len(shapes)):
