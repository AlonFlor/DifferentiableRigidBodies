...
def get_sphere_box_face_contact_point_and_normal(box, sphere):
    ...
    local_normal = geometry_utils.normalize(
        sphere_contact_point_w - box_contact_point_w)  # due to interpenetration, normal is calculated as coming out of the box
...
def shape_shape_collision_detection(collision_check_list):
    ...
    for shape1, shape2 in collision_check_list:
        ...
        if shape1.shape_type == "sphere" and shape2.shape_type == "sphere":
            ...
            shape1_dirs = [geometry_utils.rotate_only_to_world_coords(shape1, x_dir),
                           geometry_utils.rotate_only_to_world_coords(shape1, y_dir),
                           geometry_utils.rotate_only_to_world_coords(shape1, z_dir)]
            shape2_dirs = [geometry_utils.rotate_only_to_world_coords(shape2, x_dir),
                           geometry_utils.rotate_only_to_world_coords(shape2, y_dir),
                           geometry_utils.rotate_only_to_world_coords(shape2, z_dir)]
            edge_axes = []
        ...
        if can_be_collision:
            ...
            if len(this_pair_contact) > 0:
                ...
                if shape2.signed_distance(geometry_utils.to_local_coords(shape2, this_pair_contact[0]))[0] < -0.01:
                    ...
                    print("penetration:",
                          shape2.signed_distance(geometry_utils.to_local_coords(shape2, this_pair_contact[0]))[0])
